...
func TestBuildArgv_AutoTmux_Worktree_SuppressedByNoTmux(t *testing.T) {
	// Even when conditions for injection are met, --no-tmux wins.
	cfg := defaultConfig()
...
func TestBuildArgv_AutoTmux_Worktree_AlreadyInPassthrough_NotDuplicated(t *testing.T) {
	cfg := defaultConfig()
	cfg.Auto.Tmux = "worktree"
	a := Args{
		CWD:         "/p/main",
		WorktreeSet: true,
		Passthrough: []string{"--worktree", "feat", "--tmux"},
	}
	argv := buildArgv(a, "/shell", cfg)
	count := 0
	for _, tok := range argv {
...
func TestBuildArgv_AutoTmux_Worktree_ExplicitTNotDuplicated(t *testing.T) {
	// -T translates to --tmux; auto.tmux="worktree" must not add a second
	// when conditions for injection would otherwise be met.
	cfg := defaultConfig()
	cfg.Auto.Tmux = "worktree"
	a := Args{
		CWD:         "/p/main",
		WorktreeSet: true,
		Passthrough: []string{"--worktree", "feat", "--tmux", "mywin"},
	}
	argv := buildArgv(a, "/shell", cfg)
	count := 0
	for _, tok := range argv {
